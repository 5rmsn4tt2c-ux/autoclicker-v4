...
local INSTANCE_FILE = 'autoclicker-v4/profiles/yt_instance.txt'
...
function YouTube:DownloadAudio(audioUrl, filename)
	...
	local filepath = 'autoclicker-v4/youtube/' .. filename
	local ok, res = pcall(req, {
		Url = audioUrl,
		Method = 'GET',
	})
...
function YouTube:GetPlaylist(playlistId)
	if self.ApiMode == 'piped' then
		return self:Request('/playlists/' .. playlistId)
	else
		return self:Request('/api/v1/playlists/' .. playlistId)
	end
end

function YouTube:GetPlaylistTracks(data)
	if not data then return {}, nil end
	local results = {}

	if self.ApiMode == 'piped' then
		local videos = data.relatedStreams or data.videos or {}
		for _, item in videos do
			if item.url or item.videoId then
				local videoId = item.url and item.url:match('/watch%?v=(.+)') or item.videoId
				if videoId then
					table.insert(results, {
...
function YouTube:GetPlaylistNextPage(playlistId, nextpage)
	if not nextpage or self.ApiMode ~= 'piped' then return {}, nil end
	local encoded = nextpage:gsub('([^%w%-%.%_%~])', function(c)
		return string.format('%%%02X', string.byte(c))
	end)
	local data, err = self:Request('/nextpage/playlists/' .. playlistId .. '?nextpage=' .. encoded)
	if err then return {}, nil end
	return self:GetPlaylistTracks(data)
end

function YouTube:ParsePlaylistUrl(url)
	if not url or url == '' then return nil end
	local id = url:match('[?&]list=([%w%-_]+)')
	if not id then
		id = url:match('playlist/([%w%-_]+)')
	end
	if not id then
		id = url:match('^([%w%-_]+)$')
	end
