...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('autoclicker-v4/assets/new/blur.png')
...
vape.Libraries.string = loadstring(downloadFile('autoclicker-v4/libraries/string.lua'), 'string')()
...
local hash = loadstring(downloadFile('autoclicker-v4/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('autoclicker-v4/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('autoclicker-v4/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
run(function()
	...
	function whitelist:announce(text)
		...
		icon.Image = getcustomasset('autoclicker-v4/assets/new/vape.png')
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('autoclicker-v4/profiles/whitelist.json') and readfile('autoclicker-v4/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('autoclicker-v4/profiles/whitelist.json', whitelist.textdata)
				end)
			end
...
run(function()
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('autoclicker-v4/assets/new/radaricon.png'),
...
run(function()
    ...
    NoFallDamage = vape.Categories.Blatant:CreateModule({
    		...
    		if callback then
    			...
    			if Debug.Enabled then
    				NoFallDamage:Clean(entitylib.Events.LocalAdded:Connect(function(ent)
    					task.delay(1, function()
    						local stateCount = #getconnections(ent.Humanoid.StateChanged)
    						local freefallCount = #getconnections(ent.Humanoid.FreeFalling)
    						notif('NoFall Debug', 'StateChanged connections: '..stateCount..'\nFreeFalling connections: '..freefallCount, 10, 'info')
    					end)
    				end))

    				if entitylib.isAlive then
    					local stateCount = #getconnections(entitylib.character.Humanoid.StateChanged)
    					local freefallCount = #getconnections(entitylib.character.Humanoid.FreeFalling)
    					notif('NoFall Debug', 'StateChanged connections: '..stateCount..'\nFreeFalling connections: '..freefallCount, 10, 'info')
    					lastHealth = entitylib.character.Humanoid.Health
    				end
    				...
    				NoFallDamage:Clean(runService.Heartbeat:Connect(function()
    					if entitylib.isAlive then
    						...
    						if state == Enum.HumanoidStateType.Freefall and not wasFalling then
    							wasFalling = true
    							fallStartY = root.Position.Y
    							notif('NoFall Debug', 'Started falling at Y='..string.format('%.1f', fallStartY), 3, 'info')
    						end

    						if wasFalling and floor ~= Enum.Material.Air then
    							local fallDist = fallStartY - root.Position.Y
    							notif('NoFall Debug', 'Landed! Fell '..string.format('%.1f', fallDist)..' studs\nImpact vel: '..string.format('%.1f', vel)..'\nState: '..state.Name, 5, 'info')
    							wasFalling = false
    						end

    						if hum.Health ~= lastHealth then
    							local diff = hum.Health - lastHealth
    							if diff < 0 then
    								notif('NoFall Debug', 'TOOK DAMAGE: '..string.format('%.1f', -diff)..'\nHealth: '..string.format('%.1f', hum.Health)..'/'..string.format('%.1f', hum.MaxHealth)..'\nState: '..state.Name..'\nVel Y: '..string.format('%.1f', vel), 8, 'alert')
    							end
    			...
    			if Method.Value == 'Velocity' then
    				NoFallDamage:Clean(runService.PreSimulation:Connect(function()
    					if entitylib.isAlive then
    						local root = entitylib.character.RootPart
    						if entitylib.character.Humanoid.FloorMaterial == Enum.Material.Air and root.AssemblyLinearVelocity.Y < -15 then
    			...
    			elseif Method.Value == 'State' then
    				NoFallDamage:Clean(runService.PreSimulation:Connect(function()
    					if entitylib.isAlive then
    						local hum = entitylib.character.Humanoid
    			...
    			elseif Method.Value == 'Connections' then
    				local function disableConnections(ent)
    					for _, v in getconnections(ent.Humanoid.StateChanged) do
    						local info = debug.getinfo and debug.getinfo(v.Function) or {}
    						local src = (info.source or info.short_src or ''):lower()
    						if src:find('fall') or src:find('damage') or src:find('landing') then
    							table.insert(connections, v)
    							v:Disable()
    						end
    					end
    					for _, v in getconnections(ent.Humanoid.FreeFalling) do
    						table.insert(connections, v)
    						v:Disable()
    					end
    				end
    				if entitylib.isAlive then
    					disableConnections(entitylib.character)
    				end
    				NoFallDamage:Clean(entitylib.Events.LocalAdded:Connect(function(ent)
    					task.delay(1, function()
    						disableConnections(ent)
    					end)
    				end))
    			elseif Method.Value == 'Spoof' then
    				...
    				local hook = function(packet)
    					if packet.AsArray[1] == 0x1b then
    						...
    						local stateVal = buffer.readu8(data, 25) % 32
    						if stateVal == Enum.HumanoidStateType.Freefall.Value or stateVal == Enum.HumanoidStateType.FallingDown.Value then
    							buffer.writeu8(data, 25, Enum.HumanoidStateType.Running.Value + 32)
    							packet:SetData(data)
    							if Debug.Enabled then
    								notif('NoFall Debug', 'Spoofed state packet to Running', 2, 'info')
    							end
    						end
    					end
    				end
    	...
    	Tooltip = 'Prevents taking fall damage.\nVelocity - Caps fall speed while airborne\nState - Forces running state during freefall\nConnections - Disables game fall damage handlers\nSpoof - Spoofs freefall packets to server (requires raknet)',
    })
    Method = NoFallDamage:CreateDropdown({
    	Name = 'Method',
    	List = { 'Velocity', 'State', 'Connections', 'Spoof' },
    	Function = function()
    	...
    	Tooltip = 'Velocity - Caps your fall speed (slow fall)\nState - Forces running state every frame\nConnections - Disables StateChanged & FreeFalling handlers\nSpoof - Spoofs state over network (requires raknet)',
    })
    Debug = NoFallDamage:CreateToggle({
    	Name = 'Debug',
    	Function = function()
    		if NoFallDamage.Enabled then
    			NoFallDamage:Toggle()
    	...
    	Tooltip = 'Shows notifications when you fall, land, and take damage to help find the right bypass method',
    })
end)
...
run(function()
    ...
    local function Added(ent)
    	...
    	arrow.Visible = false
    	arrow.Image = getcustomasset('autoclicker-v4/assets/new/arrowmodule.png')
...
run(function()
    ...
    local function findLeaveParty()
    	for _, v in replicatedStorage:GetDescendants() do
    		if v.Name == 'leaveParty' and v:IsA('RemoteEvent') then
    			notif('LeaveParty', 'Found remote: ' .. v:GetFullName(), 10)
    			return function()
    				v:FireServer()
    			end
    		end
    	end
    	notif('LeaveParty', 'No leaveParty remote found', 10, 'warning')

    	local rbxts = replicatedStorage:FindFirstChild('rbxts_include')
    	if rbxts then
    		local fw = rbxts:FindFirstChild('node_modules')
    		fw = fw and fw:FindFirstChild('@flamework')
    		fw = fw and fw:FindFirstChild('core')
    		fw = fw and fw:FindFirstChild('out')
    		if fw then
    			local suc, Flamework = pcall(require, fw)
    			if suc and Flamework and Flamework.Flamework then
    				local ok, controller = pcall(Flamework.Flamework.resolveDependency, '@easy-games/lobby:client/controllers/party-controller@PartyController')
    				if ok and controller and controller.leaveParty then
    					notif('LeaveParty', 'Found Flamework PartyController', 10)
    					return function()
    						controller:leaveParty()
    					end
    				else
    					notif('LeaveParty', 'Flamework found but no PartyController: ' .. tostring(ok) .. ' / ' .. tostring(controller), 10, 'warning')
    				end
    			else
    				notif('LeaveParty', 'Flamework require failed: ' .. tostring(suc) .. ' / ' .. tostring(Flamework), 10, 'warning')
    			end
    		else
    			notif('LeaveParty', 'No Flamework in ReplicatedStorage', 10, 'warning')
    		end
    	else
    		notif('LeaveParty', 'No rbxts_include found', 10, 'warning')
    	end
    end
    ...
    LeaveParty = vape.Categories.Utility:CreateModule({
    	...
    	Function = function(callback)
    		if callback then
    			local leave = findLeaveParty()
    			if leave then
    				notif('LeaveParty', 'Leaving party...', 5)
    				leave()
    			else
    				notif('LeaveParty', 'No method found, teleporting to new server', 5, 'alert')
    				teleportService:Teleport(game.PlaceId)
    			end
    			LeaveParty:Toggle()
    		end
...
run(function()
    ...
    local function updateCrosshair()
        ...
        local center = inputService:GetMouseLocation()
...
run(function()
    local YTModule
    local yt = loadstring(downloadFile('autoclicker-v4/libraries/youtube.lua'), 'youtube')()
    yt:LoadSettings()

    if not isfolder('autoclicker-v4/youtube') then
        makefolder('autoclicker-v4/youtube')
    end
    ...
    local function getPlaylistPath(name)
        return 'autoclicker-v4/youtube/playlist_' .. name:gsub('[^%w_%-]', '') .. '.json'
    end
    ...
    local function listPlaylists()
        local names = {}
        pcall(function()
            for _, file in listfiles('autoclicker-v4/youtube') do
    ...
    YTModule:CreateTextBox({
        Name = 'Import YT Playlist',
        Placeholder = 'youtube playlist URL or ID',
        Function = function(val, enter)
            if not enter or not val or val == '' then return end
            local playlistId = yt:ParsePlaylistUrl(val)
            if not playlistId then
                vape:CreateNotification('YouTube', 'Invalid playlist URL/ID', 3, 'alert')
                return
            end
            vape:CreateNotification('YouTube', 'Importing playlist...', 3)
            local data, err = yt:GetPlaylist(playlistId)
            if err then
                vape:CreateNotification('YouTube', 'Import error: ' .. err .. ' - trying next instance', 3, 'alert')
                yt:TrySwitchInstance()
                data, err = yt:GetPlaylist(playlistId)
                if err then
                    vape:CreateNotification('YouTube', 'Failed: ' .. err, 4, 'alert')
                    return
                end
            end
            local tracks, nextpage = yt:GetPlaylistTracks(data)
            while nextpage and #tracks < 100 do
                local moreTracks, np = yt:GetPlaylistNextPage(playlistId, nextpage)
                if #moreTracks == 0 then break end
                for _, t in moreTracks do
                    table.insert(tracks, t)
                end
                nextpage = np
            end
            if #tracks == 0 then
                vape:CreateNotification('YouTube', 'Playlist is empty or private', 3, 'alert')
                return
            end
            trackQueue = tracks
            queueIndex = 0
            local plName = (data.name or data.title or playlistId):gsub('[^%w_%-]', '')
            if plName ~= '' then
                savePlaylist(plName, tracks)
                vape:CreateNotification('YouTube', 'Imported & saved "' .. plName .. '" (' .. #tracks .. ' tracks)', 5)
            else
                vape:CreateNotification('YouTube', 'Imported ' .. #tracks .. ' tracks', 5)
            end
            for i = 1, math.min(5, #tracks) do
            ...
            if #tracks > 5 then
                task.delay(0.75, function()
                    vape:CreateNotification('YouTube', '... and ' .. (#tracks - 5) .. ' more tracks. Tap Next to play!', 8)
                end)
            end
        end,
    })
