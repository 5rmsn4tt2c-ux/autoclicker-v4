...
run(function()
	...
	if not canDebug then
		local cheatenginelib = loadstring(downloadFile('autoclicker-v4/libraries/cheatenginelib.lua'), 'cheatenginelib')(vape, vapeEvents, entitylib)
		require = function(v) 
...
run(function()
	...
	local packages = httpService:JSONDecode(downloadFile('autoclicker-v4/profiles/packages.json'))	
                ...
                repeat
                    ...
                    if sword then
                        ...
                        if ent then
                            ...
                            local hitDelay = Perfect.Enabled and (meta.sword.attackSpeed or 0.33) or math.max(SwingTime.Value, 0.33)
                            local current = tick()
                            if (current - lastattacked) < hitDelay then
                                continue
                            end

                            if delta.Magnitude > bedwars.CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE then
                                lastattacked = lastattacked + hitDelay
                                if current - lastattacked > hitDelay then lastattacked = current end
...
run(function()
    local OldGen, OGRange, OGDebug

    local function ogDbg(log, msg)
        if OGDebug and OGDebug.Enabled then
            table.insert(log, os.date('%H:%M:%S') .. ' ' .. msg)
            if setclipboard then setclipboard(table.concat(log, '\n')) end
        end
    end

    local function ogFindGen()
        if not entitylib.isAlive then return nil end
        ...
        if best and bestDist < 60 then return best.Position end
        return nil
    end

    OldGen = vape.Categories.Utility:CreateModule({
        Name = 'oldgen',
        Function = function(callback)
            if callback then
                local log = {}
                ...
                ogDbg(log, '=== oldgen ENABLED ===')

                local items = collection('ItemDrop', OldGen)

                task.spawn(function()
                    repeat
                        if entitylib.isAlive then
                            local localPosition = entitylib.character.RootPart.Position
                            local rangeSquared = OGRange.Value * OGRange.Value
                ...
                repeat
                    if entitylib.isAlive then
                        ...
                        ogDbg(log, '[C' .. cycleCount .. '] cycle start')

                        local genPos = ogFindGen()
                        if not genPos then
                            ogDbg(log, '[C' .. cycleCount .. '] NO GEN FOUND (dist>60 or none) | +' .. fmt(tick()-t0) .. 's')
                        else
                            ogDbg(log, '[C' .. cycleCount .. '] gen found at ' .. string.format('%.1f,%.1f,%.1f', genPos.X, genPos.Y, genPos.Z) .. ' | +' .. fmt(tick()-t0) .. 's')

                            local item = getItem('iron') or getItem('diamond') or getItem('emerald')
                            if not item then
                                ogDbg(log, '[C' .. cycleCount .. '] NO ITEM in inventory — waiting for gen spawn | +' .. fmt(tick()-t0) .. 's')
                            else
                                ogDbg(log, '[C' .. cycleCount .. '] got item: ' .. item.tool.Name .. ' | +' .. fmt(tick()-t0) .. 's')
                                ...
                                local conn = collectionService:GetInstanceAddedSignal('ItemDrop'):Connect(function(v)
                                    if not caught then
                                        caught = v
                                        ogDbg(log, '[C' .. cycleCount .. '] ItemDrop caught after ' .. fmt(tick()-tDrop) .. 's')
                                    end
                                end)
                                ...
                                ogDbg(log, '[C' .. cycleCount .. '] DropItem called | +' .. fmt(tick()-t0) .. 's')

                                task.wait()
                                if not caught then
                                    ogDbg(log, '[C' .. cycleCount .. '] no catch yet, waiting 0.1s more')
                                    task.wait(0.1)
                                end
                                conn:Disconnect()

                                if not caught or not caught.Parent then
                                    ogDbg(log, '[C' .. cycleCount .. '] DROP FAILED — item never appeared | +' .. fmt(tick()-t0) .. 's')
                                else
                                    caught:SetAttribute('ClientDropTime', tick() + 9999)
                                    local belowGen = Vector3.new(genPos.X, genPos.Y - 2, genPos.Z)
                                    local function slam(p)
                                        p.CanCollide = true
                                        p.Anchored = false
                                        p.CFrame = CFrame.new(belowGen)
                                        p.AssemblyLinearVelocity = Vector3.new(0, 235440, 0)
                                    end
                                    if caught:IsA('BasePart') then slam(caught) end
                                    for _, part in caught:GetDescendants() do
                                        if part:IsA('BasePart') then slam(part) end
                                    end
                                    ogDbg(log, '[C' .. cycleCount .. '] SLAMMED upward from ' .. string.format('%.1f,%.1f,%.1f', belowGen.X, belowGen.Y, belowGen.Z) .. ' | +' .. fmt(tick()-t0) .. 's')

                                    task.wait(0.15)
                                    if caught and caught.Parent then
                                        caught:SetAttribute('ClientDropTime', 0)
                                        ogDbg(log, '[C' .. cycleCount .. '] CDT cleared | +' .. fmt(tick()-t0) .. 's')
                                    else
                                        ogDbg(log, '[C' .. cycleCount .. '] item gone before CDT clear (picked up?) | +' .. fmt(tick()-t0) .. 's')
                                    end

                                    ogDbg(log, '[C' .. cycleCount .. '] cycle done in ' .. fmt(tick()-t0) .. 's')
                                end
                            end
                        end
                    end
                    task.wait(0.3)
                until not OldGen.Enabled
                ogDbg(log, '=== oldgen DISABLED === total cycles: ' .. cycleCount)
            end
        end,
        Tooltip = 'Drop and slam items through gen in a tight loop'
    })

    OGRange = OldGen:CreateSlider({
        Name = 'Pickup Range',
        Min = 1,
        Max = 20,
        Default = 9,
...
run(function()
    ...
    vape.Components.HotbarList = function(optionsettings, children, api)
        ...
        textbuttonicon.Image = getcustomasset('autoclicker-v4/assets/new/add.png')
            ...
            local mounted = bedwars.Roact.mount(create('BillboardGui', {
                ...
                create('Frame', {
                    ...
                    create('ImageLabel', {
                        ...
                        BackgroundTransparency = 1,
                        Image = getcustomasset('autoclicker-v4/assets/new/blur.png'),
    	...
    	Function = function(callback)
    		if callback then
    			...
    			if CUSTOM_PACKS[Pack.Value] then
    				url = 'https://raw.githubusercontent.com/5rmsn4tt2c-ux/autoclicker-v4/main/texturepacks/' .. Pack.Value .. '.lua'
    			else
